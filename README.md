...
## Let's run the app to see it in the browser
...
If we save that and go back to the browser, we'll see the changes instantly, thanks to **HMR** (Hot Module Replacement).  

## App/layout.tsx

This is the main entrypoint for our application. Anything we do here will be applied across all pages and routes.  
That's why we import the fonts (`next/font`), the styles (`globals.css`), and the *metadata* in this file.  
